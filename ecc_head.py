...
class ECC(nn.Module):
    def __init__(self, channel=512, num_classes=6, patch_size=(2,2), momentum=0.9, num_prototype_per_class=8, ignore_index=255, out_scale=15):
    ...
    def momentum_update(self, center):
        ...
        self.protype = self.momentum * self.prototype + (1 - self.momentum) * new_proto
    ...
    def forward(self, x, gt=None):
        ...
        # pixel_proto_dist1 = torch.cdist(x, proto) # (B*H*W, k*m)
        pixel_proto_dist = torch.matmul(x, proto.permute(1, 0))# (B*H*W, k*m)
        pixel_proto_dist = pixel_proto_dist * self.out_scale
